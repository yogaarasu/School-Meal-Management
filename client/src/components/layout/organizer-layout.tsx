...
const navItems = [
  ...
  { to: "/daily-report", label: "dailyReport", icon: ClipboardList },
  { to: "/submissions", label: "submissions", icon: ClipboardList },
  { to: "/stock", label: "stock", icon: Package },
  { to: "/monthly-report", label: "monthlyReport", icon: BarChart3 }
] as const;

export const OrganizerLayout = ({ user, onLogout }: OrganizerLayoutProps) => {
  ...
  const profileEmail = user.organizerData?.email || user.email || "organizer@schoolmeal.tn.gov";
        ...
        >
          ...
          <Link to="/" className="rounded-xl border bg-background p-4" onClick={closeMobile}>
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-xs uppercase text-muted-foreground">{t("organizerRole")}</p>
